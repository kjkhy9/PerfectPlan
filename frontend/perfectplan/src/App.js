...
function App() {
  ...
  const addPollDate = () => setPollDates([...pollDates, ""]);
  ...
  const createPoll = async () => {
    ...
    try {
      ...
      alert(`Error: ${err.response?.data?.message || "Failed to create poll"}`);
    }
  };
  ...
  const deletePoll = async (pollId) => {
    ...
    try {
      ...
      console.error(err);
      alert(`Error: ${err.response?.data?.message || "Failed to delete poll"}`);
    }
  };
  ...
  const votePoll = async (pollId, date) => {
    try {
      ...
      if (socket) socket.emit("pollUpdate", res.data);
      setPolls((prev) => prev.map((p) => (p._id === res.data._id ? res.data : p)));
    } catch (err) {
      console.error(err);
      ...
      <div className="App">
        ...
        {isGroupOwner && (
          <div className="poll-container">
            ...
            <div className="button-group">
              <button className="btn-secondary" onClick={addPollDate}>
                + Add Date Option
              </button>
          ...
          polls.map((poll) => (
            <div key={poll._id} className="poll-container">
              ...
              {poll.isClosed && poll.winningDate && (
                <p>
                  <b>✅ Final Date: {new Date(poll.winningDate).toLocaleString()}</b>
                </p>
              )}
              ...
              {!poll.isClosed &&
                poll.options.map((opt, i) => (
                  <div key={i} className="poll-option">
                    <button
                      disabled={isGuest}
                      onClick={() => votePoll(poll._id, opt.date)}
                    >
